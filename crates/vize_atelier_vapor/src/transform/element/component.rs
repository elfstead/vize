...
pub(super) fn transform_component<'a>(
                        ...
                        if let ExpressionNode::Simple(key_exp) = arg {
                            ...
                            if kind == ComponentKind::Dynamic && key_exp.content == "is" {
                                let node = if let Some(ExpressionNode::Simple(val_exp)) =
                                    dir.exp.as_ref()
                                {
                                    SimpleExpressionNode::from_node(val_exp)
                                } else {
    ...
    if has_v_slot_on_component {
        let slot_block = ensure_sufficient_stack(|| transform_children(ctx, &el.children));
        let name_exp = SimpleExpressionNode::new("default", true, SourceLocation::STUB);
        let fn_exp = slot_props_expr.map(|expr| {
            let expr = ctx.allocator.alloc_str(&expr);
                    ...
                    && child_el.tag_type == ElementType::Template
                {
                    for prop in child_el.props.iter() {
                            ...
                            && dir.name == "slot"
                        {
                            ...
                            let slot_block = ensure_sufficient_stack(|| {
                                transform_children(ctx, &child_el.children)
                            });
                            let _template_id = ctx.next_id(); // consume ID for template wrapper
                            let slot_name = ctx.allocator.alloc_str(&slot_name);
...
pub(super) fn transform_component_v_model<'a>(
    ...
    let key_node = SimpleExpressionNode::new(prop_name_ref, true, SourceLocation::STUB);
    let key = Box::new_in(key_node, &ctx.allocator);
    let mut values = Vec::new_in(&ctx.allocator);
    let binding_ref = ctx.allocator.alloc_str(&binding);
    let val_node = SimpleExpressionNode::new(binding_ref, false, SourceLocation::STUB);
    values.push(Box::new_in(val_node, &ctx.allocator));
    props.push(IRProp {
        key,
        values,
