...
fn dynamic_key_expression<'a>(
    ...
    let expression = match directive.exp.as_ref() {
        Some(ExpressionNode::Simple(expression)) => SimpleExpressionNode::from_node(expression),
        _ => {
            let ExpressionNode::Simple(argument) = directive.arg.as_ref()? else {
                return None;
            };
            SimpleExpressionNode::new(argument.content, false, argument.loc.clone())
        }
    };
    Some(Box::new_in(expression, &ctx.allocator))
}

fn dynamic_key_source<'node, 'a>(
    element: &'node ElementNode<'a>,
    ...
    element.props.iter().find_map(|prop| {
        ...
        (argument.content == "key").then_some(&**directive)
    })
}
